...
8. **갤러리** — 반응형 이미지 슬라이드(캐러셀), 3장
9. **Contact** — 라벨-값 박스 리스트 (Email 등)
10. **Footer** — 한 줄 태그라인
...
- 반응형 분기점 760px: 히어로 세로 스택, 그리드 1열, 네비 메뉴 숨김, CTA 원 축소(88px), 슬라이드 화살표 버튼 축소(32px)

## 갤러리 슬라이드 (캐러셀)

- 가로는 컨테이너 폭에 맞춰 반응형, 세로는 `aspect-ratio: 16/7`로 고정 (필요 시 비율만 조정)
- 이미지는 `object-fit: cover`로 채움, 전체 프레임은 `border-radius: 20px` + 그림자
- 좌우 화살표 버튼(원형, 반투명 흰 배경) + 하단 도트 인디케이터로 이동
- 자동 재생 5초 간격, 마우스 오버 시 일시정지(`mouseenter`/`mouseleave`)
- 슬라이드 전환은 `.slider-track`에 `transform: translateX(-N * 100%)` + `transition`으로 구현 (외부 라이브러리 없이 순수 JS)
